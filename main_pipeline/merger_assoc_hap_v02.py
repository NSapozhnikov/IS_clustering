...
def dataframe_merger(num_chr_s, num_chr_e, meth, filename):
    pd_df = pd.DataFrame({'LOCUS':[]
                         ...
                         ,'LENGTH':[]
                         ,'SIZE':[]})
    for num_chr in range(int(num_chr_s), int(num_chr_e) + 1):
...
if __name__ == "__main__":
  ...
  parser.add_argument('-f', '--filename', 
                      help = 'a filename that is used (both for in-/out- files)', 
                      required = True)
  args = parser.parse_args()
  # Check if -n argument is sole or range
  try:
    num_chr_s = args.num_chr.split('-')[0]
    num_chr_e = args.num_chr.split('-')[1]
  except IndexError:
    num_chr_s = args.num_chr
    num_chr_e = num_chr_s
  methods = args.method
  if methods == 'all':
  ...
  for meth in methods:
    dataframe_merger(num_chr_s, num_chr_e, meth, filename)  
